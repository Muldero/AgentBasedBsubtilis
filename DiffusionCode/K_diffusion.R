...
k_e_adjust = ((k_e - min(k_e)) / (max(k_e) - min(k_e))) * 7 + 5
...
for (i in 1:(1000 * 2 * 10^6)) {
  ...
  diff_vec = flux(
    ...
    ((diff_mat$Volume[2:total] + diff_mat$Volume[1:(total - 1)]) / 2) / 3
  ...
  diff_mat$Concentration = 10^6 * diff_mat$picomoles / diff_mat$Volume

  diff_mat$Concentration[total] = K_m
  diff_mat$picomoles[total] = K_m * 10^(-6) * diff_mat$Volume[total]

  diff_mat$Concentration[1:30] = k_e_adjust[tick_count]
  diff_mat$picomoles[1:30] = k_e_adjust[tick_count] * 10^(-6) *
    diff_mat$Volume[1:30]
...
saveRDS(tick_record_mat,
        "PlotCode/Plot_Data/K_e_diffusion_trajectory.RDS")
