...
k_mat = readRDS("PlotCode/Plot_Data/K_e_diffusion_trajectory_10000.RDS")
...
k_plot_data = data.frame(
  time = (-400):800,
  k_traj = c(rep(8, 401), (k_traj_mat[, run_number] - 8) * 0.625 + 8)
)

for (repeater_var_10 in 1:20) {
  is.growing = FALSE
  ...
  for (stepwise_timer in (age + 1):(age + 800)) {
    ...
    K_m = 8 + (k_plot_data$k_traj[stepwise_timer - age + 401] - 8) *
      min((stepwise_timer - age) / 400, 1)
