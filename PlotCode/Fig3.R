...
traces = read.csv("PlotCode/Plot_Data/InVitroTraces.csv", header = TRUE)[, 2:5]
...
trackers = trackers %>% filter(Time < 2727, Time > 2490)

range_out = max(trackers$Out_tht_sig) - min(trackers$Out_tht_sig)
range_in = max(trackers$In_tht_sig) - min(trackers$In_tht_sig)

p2_SIM = ggplot(trackers, aes(x = Time - 2490)) +
  geom_line(aes(y = normalizer(In_tht_sig)*range_out + min(Out_tht_sig),
                lty = "Inner"), color = "#F8766D",
            size = 1) +
  geom_line(aes(y = Out_tht_sig, lty = "Outer"), color = "#F8766D",
            size = 1) +
  scale_y_continuous(
    name = "Outer Fraction Signaling",
    sec.axis = sec_axis(trans = ~((. - min(trackers$Out_tht_sig))/
                                    range_out)*range_in +
                          min(trackers$In_tht_sig),
...
ggsave(paste0(return_folder, "Fig3.tiff"), combined,
