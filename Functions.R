...
grow.a.biofilm2 = function(method = c("entire", "first_layer", "add_layer")) {
  ...
  if (method == "entire") {
    ...
    for (new_babies in 1:ceiling(length(border_cells[, 1])*gens_per_tick)) {
      ...
      if (exists("tht_vec")) {tht_vec[daughter] = tht_vec[parent]}
  ...
  occupied <<- occupied
  if (exists("tht_vec")) {tht_vec <<- tht_vec}
  return(atr)
}
...
find.sigs.fv = function(non = FALSE, external = FALSE, at_tab = atr) {
  # return(occupied[which(at_tab[occupied, 8] < (V_L0 - 1))])
  if (diff(range(at_tab[occupied, 8])) < (V_0 - V_L0)/4) {
    ...
    cutoff = weighted.mean(c(xval, 1), c(0.85, 0.15))# c(3/4, 1/4))
    ...
    if (length(out) > 0.95 * length(occupied)) {
      return(c())
    }
...
find.sigs.tht = function(non = FALSE, external = FALSE, tht_v = tht_vec,
                         occ = occupied, at_tab = atr) {
  # if (max(atr[occupied, 9]) <= 100 | stepwise_timer < 100) {
  #   cutoff = 0.4
  # } else {
  #   cutoff = 0.1 # median(tht_v[occ]) + sig_cutoff
  # }

  cutoff = median(tht_v[occ]) + sig_cutoff

  if (diff(range(at_tab[occ, 8])) < (V_0 - V_L0)/4) {
    # No signalers possible if the biofilm is really small
    return(c())
  } else {
    if (non == FALSE) {
      out = occ[which(tht_v[occ] > cutoff)]
    } else { # return nonsignalers
      out = occ[which(tht_v[occ] <= cutoff)]
    }
...
pot.update = function(at_tab = atr) {
  ...
  signal_kplus = F_param*(g_k/(tpp/2))*(at_tab[sigs, 8] -
                                          V_K0 * log(at_tab[sigs, 4]/at_tab[sigs, 5]))

  signal_kplus = signal_kplus * (signal_kplus > 0)

  negs = which(signal_kplus > (at_tab[sigs, 5] - 10))
  signal_kplus[negs] = (at_tab[sigs[negs], 5] - 10)


  # Step 2: Uptake
  uptake_kplus = (D_p/(tpp/2))*at_tab[, 4]*at_tab[, 7]*
    (300 - at_tab[, 5])

  # Step 3: Sum up
  at_tab[, 4] = at_tab[, 4] - uptake_kplus
  at_tab[sigs, 4] = at_tab[sigs, 4] + signal_kplus

  at_tab[, 5] = at_tab[, 5] + uptake_kplus
  at_tab[sigs, 5] = at_tab[sigs, 5] - signal_kplus
...
glu.update = function(at_tab = atr) {
  ...
  for (rank in (max(at_tab[, 9]) - 1):0) {
    ...
    # min(length(prev_ids)/length(rank_ids), 1.01)
...
mem.potential.update = function(at_tab = atr) {
  leak = V_L0 + d_l*(at_tab[, 4] - K_m)/(1 - exp(10*(K_m - at_tab[, 4])))
  leak[which(is.na(leak))] = V_L0 + d_l*0.1

  Nernst_pot =  V_K0 * log(at_tab[, 4]/at_tab[, 5])
  Nernst_pot[which(Nernst_pot == Inf)] = 0

  delta = -(g_l/(tpp/2)) * (at_tab[, 8] - leak) -
    (g_k/(tpp/2)) * (at_tab[, 8] - Nernst_pot) *
    find.sigs(at_tab) * # must have signaled
    ((at_tab[, 8] - Nernst_pot) > 0) # K+ must have been released
...
# Update ThT according to MC
tht.update = function(tv = tht_vec, at_tab = atr) {
  gain = (20/(tpp/2)) / (1 + exp(0.3*(at_tab[, 8] - V_0T)))
  decay = (10/(tpp/2))*tv
  return(tv + gain - decay)
}

V_k = function(at_tab = atr, method = c("MC", "PF"), cells) {
  # Martinez-Corral
  if (method == "MC") {
    V_K0 * log(at_tab[cells, 4]/at_tab[cells, 5])
  } else if (method == "PF") {
    -93.5 + (at_tab[cells, 4] - K_m)
  }
}
...
plot.hex.net.gg = function(remake = FALSE, # recalculate plotting coordinates?
  ...
  if (length(altcols) > 1) {
    ...
    are.unoccupied = which(tv == -1)
    occupied = which(tv != -1)

    medtemp = median(tv[occupied])
    tv = tv - medtemp + 0.05

    # This weird scaling reflects the actual effect of V on K+ uptake
    # tv[are.unoccupied] = 0
    voltage_colors = floor(tv*500)
    voltage_colors[which(voltage_colors > col_num)] = col_num
    voltage_colors[which(voltage_colors < 1)] = 1

    frame_color1 = (volt_grad[c(1:col_num)])[voltage_colors]
    point_color1 = (volt_grad[c(1:col_num)])[voltage_colors]

    frame_color1[are.unoccupied] = "#FFFFFF"
    point_color1[are.unoccupied] = "#FFFFFF"
  }

  # plot inner vs outer boundary?
  if (plot.boundary == TRUE) {
    bounds = which(atr[, 9] %in% (find.glu.radius() + c(-7:0)))
    frame_color1[bounds] = "yellow"
    point_color1[bounds] = "yellow"
  }

  # calculate plotting coordinates for cells
  # Will automatically do if code hasn't been run before, otherwise needs to be
  # told to recalculate
  ...
  p1 = ggplot(plot_df, aes(x, y)) +
    ...
    coord_fixed(ratio = 1) +# max(at_tab[occupied, 10])/max(at_tab[occupied, 11])) +
    xlim(-r, r) +
    ylim(-r, r) +
    theme_void()
  return(p1)
...
plot.singlecell.trajs = function(n = 8, do.print = TRUE) {
  if (stepwise_timer == 1) {
    cell_ids_sct <<- sample(atr[, 1], n, replace = FALSE)
    # c(sample(find.external(), n - 2),
    # sample(find.internal(), 2))
    k_i_mat <<- matrix(0, nrow = n, ncol = ticks)
    g_i_mat <<- matrix(0, nrow = n, ncol = ticks)
    v_mat <<- matrix(0, nrow = n, ncol = ticks)
  }

  k_i_mat[, stepwise_timer] <<- atr[cell_ids_sct, 5]
  g_i_mat[, stepwise_timer] <<- atr[cell_ids_sct, 7]
  v_mat[, stepwise_timer] <<- atr[cell_ids_sct, 8]

  if (do.print) {
    par(mfrow = c(3, n), mar = c(0.5, 2, 2, 1))

    pre = stepwise_timer - 1 #min(stepwise_timer - 1, 100)

    for (i in 1:n) {
      plot(g_i_mat[i, (stepwise_timer - pre):stepwise_timer],
           type = "l", main = paste("G_i", atr[cell_ids_sct[i], 9]),
           xlab = "", xaxt = "n")
    }
    for (i in 1:n) {
    ...
    # plot(trackers$Out_tht_sig[1:stepwise_timer], ylim = c(0, 0.43),
    #      type = "l", main = "Out_tht")
    # plot(trackers$Out_sig[1:stepwise_timer], type = "l", main = "Out_SIG")
    # plot(trackers$In_tht_sig[1:stepwise_timer], type = "l", main = "In_tht")
...
find.glu.radius = function(at_tab = atr) {
  ...
  # external glu = G_m
  GMV = rep(G_m, r*6 + 6)
  at_tab_test[rank_ids, 6:7] = glu.calc.uptake(GMV[1:length(rank_ids)],
...
find.internal = function(at_tab = atr) {
  int_indx = at_tab[which(at_tab[, 9] < max(glu_radius, 0) -
                            internal_adjust), 1]
...
check.whos.signaling = function(t_tab = trackers, at_tab = atr) {
  if (is.growing == FALSE) {
    temp_sigs = find.sigs.tht()
    if (length(temp_sigs)/length(occupied) >=
        trackers$Tot_tht_sig[stepwise_timer - 1]) {
...
# In: inner, Out: outer, Tot: total, Sig: signalers (defined by tht),
# Non: nonsignalers, Thr: threshold, Inher: inheritance rate,
# Var: variance, Med: median, all else mean
trackers = matrix(0, ncol = 41, nrow = ticks)
colnames(trackers) = c("Time", "Radius",
                       "In_tht_sig", "In_fv_sig", "In_sig", "In_sig_thr", "In_non_thr",
                       "In_non_tht", "In_med_V", "In_G_i", "In_K_e", "In_K_i",
                       "In_var_G_i", "In_sig_inher", "In_non_inher",
                       "Out_tht_sig", "Out_fv_sig", "Out_sig", "Out_sig_thr", "Out_non_thr",
                       "Out_non_tht", "Out_med_V", "Out_G_i", "Out_K_e",
                       "Out_K_i", "Out_var_G_i", "Out_sig_inher",
                       "Out_non_inher",
                       "Tot_tht_sig", "Tot_fv_sig", "Tot_sig", "Tot_sig_thr", "Tot_non_thr",
                       "Tot_non_tht", "Tot_med_V", "Tot_G_i", "Tot_K_e",
...
get.measure.tracker = function(ids, at_tab = atr) {
  signalers = ids[which(is.element(ids, find.sigs.tht()))]
  nonsignalers = ids[which(is.element(ids, signalers) == FALSE)]

  fv_signalers = ids[which(is.element(ids, find.sigs.fv()))]
  fv_nonsignalers = ids[which(is.element(ids, fv_signalers) == FALSE)]
  ...
  return(c(
    length(signalers)/length(ids),                         # tht_sig
    length(fv_signalers)/length(ids),                      # fv_signaling
    sum(at_tab[ids, 7] <= at_tab[ids, 2])/length(ids),     # sig
    mean(at_tab[signalers, 2]),                            # sig_thr
    mean(at_tab[nonsignalers, 2]),                         # non_thr
    mean(Eff(at_tab[nonsignalers, 8])),                    # non_tht
